...
//
// Use below for using backend as local
#terraform {
# Use local backend for now so you can learn the flow without needing cloud state infra.
#  backend "local" {}
#}
...
terraform {
  ...
  cloud {
    ...
    workspaces {
      name = "tf02-hcp-remote-state-dev"
    }
  }
}